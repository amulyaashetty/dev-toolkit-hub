...
export function diffLines(a: string, b: string): DiffLine[] {
  ...
  while (i < n && j < m) {
    if (left[i] === right[j]) {
      out.push({ type: "equal", text: left[i], leftNumber: i + 1, rightNumber: j + 1 });
      i++;
      j++;
    } else if (lcs[i + 1][j] >= lcs[i][j + 1]) {
      out.push({ type: "removed", text: left[i], leftNumber: i + 1 });
      i++;
    } else {
      out.push({ type: "added", text: right[j], rightNumber: j + 1 });
      j++;
    }
  }
